...
* Cluster
封装集群层面需要执行的任务，例如调用 Allocation 模块执行分片分配，在节点间迁移分片，集群健康检查，集群级元信息管理等任务。
集群任务执行完毕后，有可能产生新的集群状态，Cluster 模块也负责调用 Gateway 模块下发新的集群状态到其他节点。

* Allocation
实现分片的相关功能，也就是决策哪些分片应该分配到哪个节点上，哪些分片作为主分片，哪些分片作为副本分片

* Discovery
负责发现集群中的节点，并且选举主节点。
有多种不同的实现，默认的 Zen Discovery 封装了节点发现的 ping 功能和选举主节点功能。

* Gateway
负责集群元数据的存储和集群重启时元数据的恢复

* Indices
索引模块控制与索引相关的设置，这些设置针对所有索引进行全局管理，而不是在每个索引级别进行配置。具体包括  Circuit breaker、Fielddata cache、Node query cache、Indexing buffer、Shard request cache、Recovery、Search Settings

* HTTP
HTTP 模块允许通过 json over http 的方式访问 ElasticSearch API

* Transport
用于集群节点内部的通信

* Engine
封装了 lucene 和 translog 相关接口
...
1. 解析配置，包括配置文件和命令行参数
2. 加载安全配置
3. 检查内部环境，包括 lucene 版本和相关 jar 包
4. 检查外部环境，包括操作系统配置和 JVM 配置，也就是 bootstrap check
5. 启动 keepalive 线程
...
Discovery 模块负责发现集群中的节点，并且选举主节点。
有多种不同的实现，默认的 Zen Discovery 封装了节点发现的 ping 功能和选举主节点功能。

**Bully 算法**

Leader 选举的基本算法之一。它假定所有的节点都有唯一的一个 ID，使用该 ID 对节点进行排序。任何时候当前 Leader 都是参与集群的最大 ID 节点。该算法的优点是易于实现。但是，当拥有最大 ID 的节点处于不稳定状态时会有问题。例如，master 负载过重而假死，集群拥有第二大 ID 的节点被选举为新的主节点，这时原来的 master 恢复，再次被选为新主节点，然后又假死...
...
4. 维护分片元数据信息，包括哪个分片在哪个节点，最新同步数据列表 in-sync
5. 重新选举主分片后如何维护数据一致性
...
选举主分片都需要使用最新同步数据列表  in-sync ，在该表中选择
...
对于主分片来说，可能有一些数据没来得及刷盘；对于副本分片来说，一是没来得及刷盘，二是主分片写完了，副本分片还没来得及写，主副分片数据不一致。
